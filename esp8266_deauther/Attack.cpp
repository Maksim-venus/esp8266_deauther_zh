...
void Attack::start(int num) {
  ...
  if(!isRunning[num]) {
    ...
    refreshLed();
    if (debug) Serial.println("starting " + (String)attackNames[num] + " 攻击中...");
    if (num == 0) attackMode_deauth = "停止";
    else if(num == 1) attackMode_beacon = "停止";
...
void Attack::stop(int num) {
  if(isRunning[num]) {
    if (debug) Serial.println("stopping " + (String)attackNames[num] + " 攻击中...");
    if (num == 0) attackMode_deauth = "开始";
    else if(num == 1) attackMode_beacon = "开始";
    isRunning[num] = false;
    prevTime[num] = millis();
    refreshLed();
  }
...
size_t Attack::getSize(){
  if(apScan.selectedSum == 0) stati[0] = "未选中WiFi";
